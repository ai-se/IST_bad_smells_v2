...
%Note that we have already tried running such a workshop at the 2018 International Conference on Software Engineering, but  received too few submissions. 
...
\item Again, in advance undertake power calculations to determine the likelihood of the study finding an effect of the expected size.  Where possible, when there are relevant previous studies, use a bias-corrected pooled estimate \cite{Schm14} to help address likely exaggerated estimates.
...
\begin{raggedright}\smallest{NOTE: treatments have a pre-processor (`no' =  no pre-processor; `log' = use a log transform) and a learner (e.g., `SWReg' = stepwise regression; CART =  decision tree learner that post-prunes its leaves after building the tree).}%XXX should you list all the abbreviations??
\end{raggedright}
...
\caption{Using Scott-Knott, there is a better way to  representation  the Figure~\ref{bad} data.  Note that, here, it is clear that the ``SWReg'',``CART'' treatments are performing best and worst (respectively). Results from each treatment sorted by their median (and the first rows with less error are better than the last rows
...
Results  are clustered in column one using a Scott-Knott analysis.
Rows have different ranks if  Cliff's Delta reports more than a small effect difference and a bootstrap test reports significant differences.  A background gray share is  used to make different ranks stand out. 
}\label{fig:sk}
\end{figure}
...
% Implementations that use bootstrapping and a non-parametric effect size test (Cliff's Delta) to control its recursive bi-clustering are available as open source packages XXX.
 %\footnote{Url blinded for review.}%See \href{https://goo.gl/uoj3FL}{goo.gl/uoj3FL}}.  
...
%For example, in the general case, bootstrapping can be slow but when used inside a Scott-Knott procedure, it is only called $log_2(N)$ times.  Further computational speed-ups can be achieved by only calling Scott-Knott if the distributions pass an effect size test.
...
\begin{enumerate}
    \item Read more, reflect more. Study the methods of SE science.%XXX what's this?
...
\RED {\bf  Comment:  Tim, do you think the reviewer has a point.  A simple way to handle is just to drop this recommendation???? } \BLACK
